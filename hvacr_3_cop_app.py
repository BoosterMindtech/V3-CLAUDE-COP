...
def main():
    ...
    with st.sidebar:
        ...
        input_mode=st.radio("Modo entrada",["Manual","API BMS/SCADA/GMAO"])
        st.divider()
        ...
        if input_mode=="API BMS/SCADA/GMAO":
            st.divider(); st.subheader("🔌 API REST BMS/SCADA/GMAO")
            gen_url=st.text_input("URL endpoint")
            api_key=st.text_input("API Key",type="password")
            fmap=st.text_area("Mapeo JSON",'{"P_evap_bar":"LP","P_cond_bar":"HP"}',height=80)
        st.caption("v1.1 · IEA HPT Annex 52")
    ...
    with tab1:
        ...
        elif input_mode=="API BMS/SCADA/GMAO":
            do_calc=st.button("🔄 Obtener datos BMS/SCADA/GMAO y calcular",type="primary")
            if do_calc:
                try:
